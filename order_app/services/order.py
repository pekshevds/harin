...
def handle_order(order_dict: dict) -> Order:
    changed = False
...
def handle_items_order(items_list: list, order: Order) -> None:
    ...
    for item_dir in items_list:
        ...
        if item_order is None:
            item_order = ItemOrder.objects.create(id=item_order_id, order=order)
            changed = True
...
def handle_order_list(order_list: dict) -> QuerySet[Order]:
    orders_id = []
    with transaction.atomic():
        for order_item in order_list:
            order = handle_order(order_dict=order_item)
